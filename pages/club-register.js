...
const fonts = `
  ...
  @keyframes reg-countdown { from{width:100%} to{width:0%} }
...
function SubmittedPage({ clubName }) {
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", background:"radial-gradient(ellipse at 20% 20%, #ede9fe 0%, #f5f0ff 30%, #faf8ff 60%, #ffffff 100%)", position:"relative" }}>
      ...
      <svg aria-hidden="true" style={{ position:"fixed", inset:0, width:"100%", height:"100%", pointerEvents:"none", zIndex:0, opacity:.5 }}>
        <defs><pattern id="sc-dots" width="26" height="26" patternUnits="userSpaceOnUse"><circle cx="1.5" cy="1.5" r="1" fill="#7c3aed" fillOpacity="0.07"/></pattern></defs>
        <rect width="100%" height="100%" fill="url(#sc-dots)"/>
      </svg>

      <div style={{ height:3, background:"linear-gradient(90deg,#4c1d95,#7c3aed,#c4b5fd,#7c3aed,#4c1d95)", position:"relative", zIndex:2 }}/>
      <nav style={{ padding:"20px 36px", position:"relative", zIndex:2 }}>
        <a href="/page" style={{ display:"inline-flex", alignItems:"center", gap:10, textDecoration:"none" }}>
          <div style={{ width:32, height:32, borderRadius:10, background:"linear-gradient(135deg,#1a0533,#3b0764)", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 14px rgba(26,5,51,0.25)" }}>
            <span style={{ color:"#c4b5fd", fontWeight:800, fontSize:15, fontFamily:"'Fraunces',serif" }}>D</span>
          </div>
          <span className="reg-display" style={{ fontSize:19, color:"#1a0533", letterSpacing:"-0.02em" }}>
        ...
        <div style={{ width:"100%", maxWidth:520, animation:"reg-fadeUp .5s cubic-bezier(.22,1,.36,1) both" }}>
          <div className="reg-success-card">
            ...
            <div style={{ position:"relative", width:88, height:88, margin:"0 auto 28px", zIndex:1 }}>
              ...
              <div style={{ position:"absolute", inset:0, borderRadius:"24px", background:"linear-gradient(145deg,#dcfce7,#bbf7d0)", border:"2px solid rgba(34,197,94,0.2)", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 16px 48px rgba(34,197,94,0.18),inset 0 1px 0 rgba(255,255,255,0.9)", animation:"reg-pop .5s cubic-bezier(.22,1,.36,1) both" }}>
            ...
            <div style={{ position:"relative", zIndex:1 }}>
              <h1 className="reg-display" style={{ fontSize:28, fontWeight:800, color:"#1a0533", letterSpacing:"-0.03em", lineHeight:1.2, margin:"0 0 10px" }}>
                Клуб бүртгэгдлээ! 🎉
              </h1>
              ...
              <p className="reg-sans" style={{ fontSize:13.5, color:"#9879d4", lineHeight:1.7, margin:"0 0 32px" }}>
                Имэйл хаяг руу баталгаажуулах линк илгээлээ. Линкийг дарж эхлээрэй.
              </p>
                ...
                {steps.map((s, i) => (
                  <div key={i} style={{ display:"flex", gap:14, alignItems:"flex-start", paddingBottom: i < steps.length-1 ? 16 : 0 }}>
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flexShrink:0 }}>
                      <div style={{ width:28, height:28, borderRadius:"50%", background: s.done ? "linear-gradient(135deg,#22c55e,#16a34a)" : "rgba(124,58,237,0.06)", border:`1.5px solid ${s.done ? "#22c55e" : "rgba(124,58,237,0.14)"}`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow: s.done ? "0 4px 12px rgba(34,197,94,0.22)" : "none", flexShrink:0 }}>
                        {s.done
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12, alignItems:"center" }}>
                <a href="/page1" className="reg-btn-primary" style={{ width:"100%", maxWidth:300 }}>
          ...
          <p className="reg-sans" style={{ textAlign:"center", fontSize:12, color:"#c4b5fd", marginTop:24, fontWeight:500, lineHeight:1.6 }}>
            Асуух зүйл байвал{" "}
...
export default function ClubRegisterPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (form.pricingType === "paid" && form.tiers.length > 0)
        fd.append("tiers", JSON.stringify(form.tiers));
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                <div>
                  ...
                  <input style={inp} placeholder="e.g. Ulaanbaatar FC" value={form.clubName}
                    onChange={e => setField("clubName", e.target.value)} {...focusHandlers} />
                </div>
                <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                <div>
                  ...
                  <input style={inp} placeholder="e.g. Суурин 4, Сүхбаатар дүүрэг" value={form.address}
                    onChange={e => setField("address", e.target.value)} {...focusHandlers} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input style={inp} type="email" placeholder="club@email.com" value={form.email}
                    onChange={e => setField("email", e.target.value)} {...focusHandlers} />
                </div>
